...
public class AdminDAO {
    ...
    public Admin checkLogin(String mail, String matkhau) throws SQLException {
        Connection conn = null;
        PreparedStatement ptm = null;
        ResultSet rs = null;
        String sql = "  SELECT * from Admin\n"
                + " WHERE UsernameAdmin=? AND PassAdmin=? ";
        try {
            conn = db.getConn();
            if (conn != null) {
                ...
                if (rs.next()) {
                    ...
                    String _Name = rs.getString("UsernameAdmin");
                   
                    String _Email = rs.getString("MailAdmin");
                    String _Pass = rs.getString("PassAdmin");
                    String _RoleID = rs.getString("RoleID");
                    Role _RoleIDAdmin = dao.details(_RoleID);
                    return (Admin) new Admin(_Name, _Pass, _Email, _RoleIDAdmin);
//                    return (TaiKhoan)new TaiKhoan(rs.getString(1), rs.getString(2), rs.getBoolean(3), rs.getString(4),rs.getObject(email, type));
    ...
    public void update(Admin edittedItem) {
        try {
            String sql = "UPDATE Admin SET PassAdmin=?, MailAdmin=? WHERE UsernameAdmin=?";
            PreparedStatement stmt = db.getConn().prepareStatement(sql);

            stmt.setString(1, edittedItem.getPassAdmin());
            stmt.setString(2, edittedItem.getMailAdmin());
            stmt.setString(3, edittedItem.getUsernameAdmin());
