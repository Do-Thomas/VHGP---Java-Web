                                    ...
                                    <c:forEach items="${requestScope.LIST_ORDER_CUSTOMER}" var="o" varStatus="counter">
                                        ...
                                        <form action="DivideWorkController" method="POST"> 
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" name="EmployeeName" list="browsers">
                                                    <datalist id="browsers">
                                                        <c:forEach items="${requestScope.LIST_EMPLOYEE_1}" var="e1" varStatus="counter">
                                                            <option value="${e1.employeeName}"></option>
                                                        </c:forEach>
                                                    </datalist>
                                                </td>
...
<!--                                                <td><input type="datetime-local" value="${o.orderDetail.timeEnd}"></td>-->
                                            <td>${o.status.statusName}</td>
                                            <td><span class="badge badge-dot mr-4">
<!--                                                        <i class="bg-warning"><a href="${up}" class="btn btn-primary py-2 px-3"><i class="fa fa-edit"></i></a></i>-->
                                                    <input name="submit" type="submit" value="Up" onclick="return validateDeliveryTime()"/>
                                                    <a onclick="myFunction()" href="CancelManage?OrderID=${o.orderID}">Hủy</a> 
                                                </span>
                                            </td>
                ...
                <div class="table-responsive">
                    <table class="table align-items-center table-flush table-nhidesgin">
                        <div class="table-column">
                            ...
                            <c:if test="${requestScope.LIST_EMPLOYEE_1 != null}">
                                <c:if test="${ not empty requestScope.LIST_EMPLOYEE_2}">
                                    <tbody>
                                        <%
                                            ArrayList<Employee> Employee = (ArrayList<Employee>) request.getAttribute("LIST_EMPLOYEE_2");
                                        %>
                                        ...
                                        <tr>
                                            <td>
                                                <b><%=emp.getEmployeeName()%></b>
                                            </td>
                                            <td><b><%=emp.getEmployeePhone()%></b></td>
                                            <%
                                                int EmployeeID = emp.getEmployeeID();
                                                OrdersDAO OrderDAO = new OrdersDAO();
                                                List<Order> od = OrderDAO.getOrderList(EmployeeID);

                                                for (Order o : od) {
                                                    SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss.SS aa");
                                                    // Format the date as a string
                                                    String timeStart = dateFormat.format(o.getOrderDetail().getTimeStart());
                                                    String timeEnd = dateFormat.format(o.getOrderDetail().getTimeEnd());
