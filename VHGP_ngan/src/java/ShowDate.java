 ...
 * @author ASUS
 */
@WebServlet(urlPatterns = {"/ShowDate"})
public class ShowDate extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String idss = request.getParameter("deliveryTime");
            HttpSession session = request.getSession();
            Cart cart = (Cart) session.getAttribute("CART");
            session.setAttribute("CART", cart);
            session.setAttribute("madv", id);
            session.setAttribute("deliveryDate", ids);

            session.setAttribute("delivery", idss);
            url = SUCCESS;
        } catch (Exception e) {
